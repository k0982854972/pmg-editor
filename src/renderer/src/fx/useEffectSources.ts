    ...
    (doc: MeshdescDocument): void => {
      ...
      const run = async (): Promise<void> => {
        ...
        const plan = planEffectSourceAutoLoad({
          ...
          freeSlots: MAX_EFFECT_SOURCES - current.length
        })
        ...
        const merged = [...before, ...loaded.filter((entry) => !hasPath(before, entry.path))].slice(
          0,
          MAX_EFFECT_SOURCES
        )
        const appendedCount = merged.length - before.length
        if (appendedCount > 0) applySources(merged)
        setStatus(autoLoadStatusText(appendedCount, plan.missingNames, plan.isTruncated))
      }
      run().catch((error) => {
